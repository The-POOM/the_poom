...
/*
 * PCAP magic numbers are defined by their on-disk byte order.
 *
 * On little-endian targets (ESP32), writing a uint32_t directly to a file will
 * store the value in little-endian byte order. To ensure the correct magic
 * bytes appear on disk, use the swapped constants below:
 * - Little-endian file: bytes d4 c3 b2 a1 -> value 0xA1B2C3D4 when written on LE
 * - Big-endian file:    bytes a1 b2 c3 d4 -> value 0xD4C3B2A1 when written on LE
 */
#define PCAP_MAGIC_BIG_ENDIAN 0xD4C3B2A1    /*!< Big-Endian (on-disk a1 b2 c3 d4) */
#define PCAP_MAGIC_LITTLE_ENDIAN 0xA1B2C3D4 /*!< Little-Endian (on-disk d4 c3 b2 a1) */
