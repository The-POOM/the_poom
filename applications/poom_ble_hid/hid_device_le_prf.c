...
void esp_hidd_prf_cb_hdl(esp_gatts_cb_event_t event,
  ...
  switch (event) {
    ...
    case ESP_GATTS_WRITE_EVT: {
      ...
      if (param->write.handle ==
        ...
        (hidd_le_env.hidd_cb)(ESP_HIDD_EVENT_BLE_LED_REPORT_WRITE_EVT,
                              &cb_param);
      }
#if (SUPPORT_REPORT_VENDOR == true)
      if (param->write.handle ==
              hidd_le_env.hidd_inst
                  .att_tbl[HIDD_LE_IDX_REPORT_VENDOR_OUT_VAL] &&
...
void hidd_set_attr_value(uint16_t handle,
                         uint16_t val_len,
                         const uint8_t* value) {
